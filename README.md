...
Ideally, you'll have the luxury of re-writing your applications to utilize the cloud smartly, meaning that
the application is design from the start to only consume the cloud features that are needed. 
Developing such takes time -- so this should be part of your longer term plans.
...
So ideally, you should only use the minimal amount of compute and storage required. Simple, yes?
...
These are some of the best techniques for lift-and-shift workloads, and don't require a lot of work or an application re-write.
...
Azure tends to be less expensive
